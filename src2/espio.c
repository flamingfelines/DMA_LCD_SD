...
void espio_bus_init(int miso, int mosi, int sck, int dma_chan) {
    ...
    buscfg.max_transfer_sz = 240*240*2; // full RGB565 frame
...
// ========= MicroPython binding =========
STATIC mp_obj_t mp_espio_bus_init(size_t n_args, const mp_obj_t *args) {
    int miso = mp_obj_get_int(args[0]);
    int mosi = mp_obj_get_int(args[1]);
    int sck  = mp_obj_get_int(args[2]);
    int dma  = mp_obj_get_int(args[3]);

    espio_bus_init(miso, mosi, sck, dma);
    return mp_const_none;
}
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(espio_bus_init_obj, 4, 4, mp_espio_bus_init);

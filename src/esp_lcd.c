...
static mp_obj_t esp_lcd_spi_bus_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args)
{
    enum {
        ARG_esp_spi_bus,
        ARG_spi_host,
    ...
    esp_err_t ret = esp_lcd_new_panel_io_spi(
        (esp_lcd_spi_bus_handle_t)(self->spi_host),
        &io_config,
        &self->io_handle
    );
    if (ret != ESP_OK) {
